...
- Added option to publish stubs for modules `php artisan vendor:publish --provider="Mhmmdkyr\Modules\LaravelModulesServiceProvider" --tag="stubs"`
...
- Fixed bug: `Target class [Mhmmdkyr\Modules\Commands\] does not exist.`
...
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Mhmmdkyr\Modules\Module`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Mhmmdkyr\Modules\Module`.
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Mhmmdkyr\Modules\FileRepository`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Mhmmdkyr\Modules\FileRepository`.
- **BREAKING** Removed the `__get` magic method on the `\Mhmmdkyr\Modules\Module` class. Use `get()` or `json()->get()` instead.
